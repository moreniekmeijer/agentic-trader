...
class PortfolioDecision(BaseModel):
    action: Literal["HOLD", "TIGHTEN_STOP", "REDUCE", "EXIT", "CLOSE_EARLY"]
    reasoning: list[str]
...
Your task is to review the currently OPEN position for {symbol} and decide whether to HOLD,
TIGHTEN_STOP, REDUCE, or EXIT.
...
2. Your ONLY job is de-risking. You may never recommend increasing size.
3. If the original thesis still holds, or if the trade just needs more time to play out, you MUST choose HOLD.
4. If risk has risen but the thesis is not broken, choose TIGHTEN_STOP or REDUCE.
5. If the thesis is invalidated or risk is unacceptable, choose EXIT.

Provide your final decision as a JSON object matching this schema:
{{
  "action": "HOLD" | "TIGHTEN_STOP" | "REDUCE" | "EXIT",
...
"""
        ...
        try:
                ...
                messages=[
                    {
                        "role": "system",
                        "content": (
                            "You are a logical, risk-aware portfolio manager AI. "
            ...
            return PortfolioDecision(action="HOLD", reasoning=[f"LLM Error: {str(e)}. Defaulting to HOLD."])
