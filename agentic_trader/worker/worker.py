...
async def handle_position_review(event, bus: EventBus) -> None:
    ...
    def _review():
        snapshot = sync_broker_snapshot(alpaca_controller)
        if snapshot is None:
        ...
        with get_session() as session:
            ...
            for pos in positions:
                ...
                unrealized_pnl_pct = float(pos.unrealized_plpc or 0.0)
...
async def run_scheduler(bus: EventBus):
    ...
    await asyncio.to_thread(sync_broker_snapshot, alpaca_controller)
    ...
    while True:
        ...
        if trade_timer >= TRADE_INTERVAL:
            snapshot = await asyncio.to_thread(sync_broker_snapshot, alpaca_controller)
            if snapshot is None:
                ...
                if symbols:
                    await bus.publish(
                        BatchAnalysisRequestedEvent(timestamp=datetime.now(timezone.utc), symbols=symbols)
                    )
